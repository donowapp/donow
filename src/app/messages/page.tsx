...
function Initials({ name }: { name: string }) {
  ...
  const letters = parts.length >= 2
    ...
    : name.slice(0, 2);
  return (
    <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full bg-violet-100 text-sm font-bold text-violet-700 uppercase">
      {letters}
    </div>
  );
}
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {conversations.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-lg bg-white py-20 shadow text-center">
            ...
            <Link href="/donations" className="mt-6 inline-block rounded-lg bg-violet-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-violet-700">
              Browse Donations
            </Link>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-violet-600 font-medium truncate">{conv.donationTitle}</p>
                  ...
                  {unread > 0 && (
                    <span className="flex-shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-violet-600 text-xs font-bold text-white">
                      {unread > 9 ? '9+' : unread}
                    </span>
                  )}
                </Link>
