...
export function Input({ label, id, type = 'text', value, onChange, placeholder, error, className = '', required = false }: InputProps) {
  ...
  return (
    <div className="w-full mb-4">
      ...
      <input id={inputId} type={type} value={value} onChange={onChange} placeholder={placeholder} className={`w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none transition ${className}`} />
